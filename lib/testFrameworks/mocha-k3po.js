...
module.exports = Mocha.interfaces['mocha-k3po'] = function (suite) {
    ...
    var ignoredBrowsers = this.options.ignoredBrowsers;
    suite.on('pre-require', function (context, file, mocha) {
        ...
        context.describe = context.context = function (title, fn) {
            ...
            // check to determine whether test suite should be ignored
            if (ignoredBrowsers.indexOf(process.env.browser+" "+process.env.version) !== -1) {
                suite.pending=true;
            }
            fn.call(suite);
            suites.shift();
        ...
        context.it = context.specify = function (title, fn) {
            ...
            var test = new Test(title, function (done) {
                ...
                k3po.prepare(scripts, browserRunner.getOrigin()).then(function () {
        ...
        suite.afterAll(function (done) {
            ...
            // Used to set the passed/failed status in SauceLabs
            if(process.env.SAUCE_USERNAME) {
                var sessionID = context.browserConfig.getSessionID();

                var sauceConnection = new SauceLabs({
                username: process.env.SAUCE_USERNAME,
                password: process.env.SAUCE_ACCESS_KEY
                });

                if(failedTests === 0) {
                    sauceConnection.updateJob(sessionID, { passed: true }, function () {});
                }  else {
                    sauceConnection.updateJob(sessionID, { passed: false }, function () {});
                }
            }
